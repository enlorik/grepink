...
void main() {
  group('SearchScreen claim review polish', () {
    testWidgets(
        'generate draft button is disabled with no selected saveable claims '
        'and enables once one is selected', (tester) async {
      ...
      // Contradictions are not selected by default, so nothing is selected.
      expect(container.read(claimReviewProvider).selection!.selectedIds, isEmpty);
      ...
      expect(
        find.byKey(const Key('generate-draft-disabled-helper')),
        findsOneWidget,
      );
    ...
    testWidgets('helper text explains new claims are selected by default and '
        'contradictions need manual review', (tester) async {
      final provider = _FixedGroundedAnswerProvider(
        GroundedAnswer(
          question: 'q',
          answerText: 'answer',
